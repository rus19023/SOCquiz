...
var selectGame = ( gameChoice ) => {
    ...
    if (fileName.includes('cyb')) {
        document.documentElement.style.setProperty('--theme', '#f4bf75');
    } else {
    ...
    fetch(jsonFile)
        .then((res) => {
            ...
            // Call the function to get random objects
            const randomObjects = getRandomObjects(res, MAX_QUESTIONS);
    ...
    getRandomObjects = (jsonData, count) => {
        // Check if the desired count is greater than the array length
        if (count > jsonData.length) {
            console.log('Count exceeds the array length.');
            return null;
        }
        ...
        // Generate random indices and retrieve corresponding objects
        const indices = [];
        while (indices.length < count) {
            const randomIndex = Math.floor(Math.random() * jsonData.length);
            if (!indices.includes(randomIndex)) {
                indices.push(randomIndex);
                selectedObjects.push(jsonData[randomIndex]);
            }
        }
